...
describe("game-persistence", () => {
  ...
  it("saveGameState 返回存档目录路径，目录名包含时间戳前缀", () => {
    const state = initializeGame("persist-1", createStage(DEV_STAGE_LAYOUT), DEFAULT_PLAYER);
  ...
  it("存档目录包含 state.json", () => {
    const state = initializeGame("persist-2", createStage(DEV_STAGE_LAYOUT), DEFAULT_PLAYER);
    const savedDir = saveGameState(state, tmpDir);
    expect(fse.existsSync(path.join(savedDir, "state.json"))).toBe(true);
  });

  it("存档目录为每个 agent 生成 .json 与 _buffer.md", () => {
    const state = initializeGame("persist-agents", createStage(DEV_STAGE_LAYOUT), DEFAULT_PLAYER);
    const savedDir = saveGameState(state, tmpDir);
  ...
  it("_buffer.md 使用 agent.name 作为 AI 前缀，System 前缀保持不变", () => {
    const state = initializeGame("buffer-prefix", createStage(DEV_STAGE_LAYOUT), DEFAULT_PLAYER);
    const savedDir = saveGameState(state, tmpDir);
  ...
  it("saveGameState 若目录不存在则自动创建", () => {
    ...
    const state = initializeGame("nested", createStage(DEV_STAGE_LAYOUT), DEFAULT_PLAYER);
    saveGameState(state, nestedDir);
    expect(fse.readdirSync(nestedDir).length).toBeGreaterThan(0);
  });

  it("多次 save 产生多个子目录", async () => {
    const state = initializeGame("multi", createStage(DEV_STAGE_LAYOUT), DEFAULT_PLAYER);
  ...
  it("loadGameState 能从目录还原 GameState", () => {
    const state = initializeGame("load-1", createStage(DEV_STAGE_LAYOUT), DEFAULT_PLAYER);
    const savedDir = saveGameState(state, tmpDir);
  ...
  it("保存后再修改状态，loadGameState 读出的仍是保存时的快照", () => {
    const state = initializeGame("snapshot-test", createStage(DEV_STAGE_LAYOUT), DEFAULT_PLAYER);
    const savedDir = saveGameState(state, tmpDir);
    applyReveal(state, 0, 0);
    const loaded = loadGameState(savedDir);
  ...
  it("loadGameState 还原的 agents 是 GameAgent 实例（有方法）", () => {
    const state = initializeGame("agents-restored", createStage(DEV_STAGE_LAYOUT), DEFAULT_PLAYER);
    const savedDir = saveGameState(state, tmpDir);
    const loaded = loadGameState(savedDir);
