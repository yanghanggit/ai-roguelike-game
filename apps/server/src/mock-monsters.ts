...
- **strike**：对目标发动攻击，同时附上一句简短的攻击描述（供日志展示），调用后本回合立即结束。
- 若本回合无需行动，直接停止输出即可（finish_reason=stop）。

**距离规则（重要）**

本游戏规则层面**不存在距离与位置**的概念——怪物与玩家始终处于可交战状态。叙事上可以有距离感描写（如"远处的身影"），但**不得以"距离太远"为理由推迟或放弃 strike**。只要决定攻击，随时可以调用 strike。`;
