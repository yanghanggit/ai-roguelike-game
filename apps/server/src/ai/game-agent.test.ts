...
describe("GameAgent 构造", () => {
  it("name 被正确赋值", () => {
    const agent = new GameAgent("slime-01", "史莱姆", "你是一只史莱姆。");
  ...
  it("两个 agent 的 context 彼此独立", () => {
    const a = new GameAgent("a", "A", "提示 A");
    const b = new GameAgent("b", "B", "提示 B");
...
describe("think() — 正常响应", () => {
  ...
  it("返回 AI 的行动描述字符串", async () => {
    const agent = new GameAgent("slime", "史莱姆", "你是一只史莱姆。");
...
describe("thinkBatch() — 正常响应", () => {
  ...
  it("返回与 agents 顺序一致的行动字符串数组", async () => {
    const slime = new GameAgent("slime", "史莱姆", "你是史莱姆。");
    const skeleton = new GameAgent("skeleton", "骷髅", "你是骷髅。");
